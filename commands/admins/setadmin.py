...
async def settadmin_handlers(bot, message: types.Message):
    if len(message.text.split()) < 3:
        await message.reply("<blockquote>Использование: /setadmin «@тег» (уровень)</blockquote>", parse_mode="HTML")
        return
    ...
    if initiator_level < 3:
        await message.reply("<b>⛔ У вас недостаточно прав для назначения админов.</b>", parse_mode="HTML")
        return
    
    # Проверка на попытку повысить до своего или более высокого уровня
    ...
    async with async_session() as session:
        # Получаем user_id по username в рамках конкретной группы
        ...
        # Проверка на попытку понизить уровень (должна использоваться команда delladmin)
        if new_level < current_target_level:
            await message.reply(
                f"<b>⛔ Эта команда предназначена только для повышения уровня администрирования.\n\nДля понижения используйте /delladmin (текущий уровень администрирования: {current_target_level})</b>",
                parse_mode="HTML"
            )
            return
    ...
    await message.reply(f"<b>📈 Пользователь @{username} повышен на {new_level} уровень администрирования.</b>", parse_mode="HTML")
    await bot.send_message(
            dew_channel, 
            f"<b>👥  Группа: <code>{message.chat.title}</code></b>\n\n"
            f"<b>👤 Пользователь <a href='tg://user?id={message.from_user.id}'>{message.from_user.first_name}</a> использовал команду <code>/setadmin</code></b>\n"
            f"<b>💠 id пользователя: <code>{message.from_user.id}</code></b>\n\n"
            f"<b>✉️ Полученное сообщение:</b>"
            f"<blockquote>📈 Пользователь {username} повышен на {new_level} уровень администрирования</blockquote>",
            parse_mode="HTML", 
